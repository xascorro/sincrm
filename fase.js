...
$(".edita_fila").live('click', function() {
	var id = $(this).attr('id');
	$("#fila"+id).load("fase_edita_fila.php", "id="+id);
	$("#id_modalidad").focus();
});
...
$(".desedita_fila").live('click', function() {
	var id = $(this).attr('id');
	$("#fila"+id).load("fase_desedita_fila.php", "id="+id);
});
...
$(".borra_fila").live('click', function() {
	...
	//borro entrada de DB
	//TODO
	$("#fila"+id).load("fase_borra_fila.php", "id="+id);
	//elimino fila
	$("#fila"+id).remove();
});
...
$("#editable-sample_new").live('click', function() {
	...
	$("#nueva_fila").load("fase_edita_fila.php", "id=nueva_fila");
	$("#modalidades").focus();
...
$(".guarda_fila").live('click', function() {
	...
	if(id == "undefined") 
		id = "nueva_fila";
	var id_modalidad = $('#modalidades').val();
	...
	$.ajax({
	     type:'POST',
	      url:'./fase_guarda.php',
	      data:'id='+id+'&id_modalidad='+id_modalidad+'&id_categoria='+id_categoria+'&orden='+orden,
	...
	//actualiza una fila para mostrar contenido no editable
	$("#fila"+id).load("fase_lee_fila.php", "id="+id+"&id_modalidad="+id_modalidad+"&id_categoria="+id_categoria+"&orden="+orden);
	$("#nueva_fila").load("fase_lee_fila.php", "id=new&id_modalidad="+id_modalidad+"&id_categoria="+id_categoria+"&orden="+orden);
    setTimeout(function() {
