...
$pdf->AddPage();
	...
	while($rutina = mysql_fetch_array($rutinas)){
		...
		if($titulo =='Orden de salida'){
			...
			$html .='<tr style="font-size:+15, font-weight:bold; background-color:'.$rutina_color.'"><td>'.$nombre_rutina.'</td><td colspan="2" style="text-align:right;">'.$musica.'<em style="">'.$rutina['orden'].'</em></td></tr>';
		}elseif($titulo =='Rutinas'){
			$preswimmer = '';
			if($rutina['orden']=='-1')
				$preswimmer = " (PRESWIMMER)";
			$html .='<tr style="font-size:+15, font-weight:bold; background-color:'.$rutina_color.'"><td>'.$nombre_rutina.$preswimmer.'</td><td colspan="2" style="text-align:right;">'.$musica.'</td></tr>';
	}
		//fin segun titulo documento
