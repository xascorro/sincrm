...
$GLOBALS['footer_substring'] = "Sede: ".$GLOBALS['lugar']."\n-- Fecha de la competición: ".dateAFecha($GLOBALS['fecha']);
...
while($fase = mysql_fetch_array($fases)){
	...
	if($titulo =='Orden de salida'){
		$html .= '<thead><tr><th width="50%"><h4>Nombre</h4></th><th width="25%"><h4>Año</h4></th><th width="25%" aling="right"><h4>Orden de salida</h4></th></tr></thead>';
		$order_by = " order by orden";

	}elseif($titulo =='Rutinas'){
	$html .= '<thead><tr><th width="50%"><h4>Nombre</h4></th><th width="25%"><h4>Año</h4></th><th width="25%" aling="right"><h4></h4></th></tr></thead>';
	$order_by = "";
}
	...
	while($rutina = mysql_fetch_array($rutinas)){
		...
		if($titulo =='Orden de salida'){
			...
			$html .='<tr style="font-size:+16, font-weight:bold; background-color:'.$rutina_color.'"><td>'.$nombre_rutina.'</td><td colspan="2" style="text-align:right;">'.$musica.'<em style="">'.$rutina['orden'].'</em></td></tr>';
		}elseif($titulo =='Rutinas'){
			$html .='<tr style="font-size:+16, font-weight:bold; background-color:'.$rutina_color.'"><td>'.$nombre_rutina.'</td><td colspan="2" style="text-align:right;">'.$musica.'</td></tr>';
	}
		//fin segun titulo documento
		...
		while($participante = mysql_fetch_array($participantes)){
				...
				$fecha_nacimiento_participante=dateAFecha(mysql_result(mysql_query($query),0,3));
				$titular = "";
				if($participante['reserva']=='si')
					$titular = "(R)";
						//segun titulo de documento
				if($titulo =='Orden de salida'){
					$html .='<tr style="background-color:'.$rutina_color.'"><td width="50%">&nbsp;&nbsp;'.$apellidos_participante.', '.$nombre_participante.' '.$titular.'</td><td width="25%">'.substr($fecha_nacimiento_participante,6).'</td><td width="25%" style="text-align:right;"></td></tr>';
				}elseif($titulo =='Rutinas'){
					$html .='<tr style="background-color:'.$rutina_color.'"><td width="50%">'.$apellidos_participante.', '.$nombre_participante.' '.$titular.'</td><td width="25%">'.substr($fecha_nacimiento_participante,6).'</td><td width="25%" style="text-align:right;"></td></tr>';
			}
				//fin segun titulo documento
