...
// -----------------------------------------------------------------------------
//saco fases_figuras
$query = "select id_categoria, id_modalidad, id, orden from fases where id_competicion ='".$GLOBALS["id_competicion_activa"]."' order by orden";
    ...
    //saco numeros de juez
   	...
   	while($juez = mysql_fetch_array($jueces)){
	   	...
	   	while($orden = mysql_fetch_array($ordenes)){
			...
			$html .= '<tr><th colspan="2">'.'<span>'.$juez['nombre'].' - '.$nombre_modalidad.'</span>'.'</th></tr>';
