...
while($fase = mysql_fetch_array($fases)){
	...
	while($rutina = mysql_fetch_array($rutinas)){
		...
		if($par%2==0)
			...
			while($orden = mysql_fetch_array($ordenes)){
				if($orden['orden'] < 10 && $orden['orden'] > 0)
					$orden['orden'] = '0'.$orden['orden'];
				if($orden['orden'] == '-1')
					$text_orden = 'PRESWIMMER  ';
				else
	...
	$html .= '<tr><th colspan="5"><h4>Cortes</h2></th></tr>';
	//$html .= '<tr><th colspan="5">';
	$query = "select * from fases_figuras where id_categoria = '".$fase['id_categoria']."' and id_competicion = '".$GLOBALS["id_competicion_activa"]."'";
	...
	while($corte = mysql_fetch_array($cortes)){
		...
		$nombre_nadadora .= ", ".mysql_result(mysql_query($query),0);
		$html .= 'Figura '.$figura.' = '.$nombre_nadadora.'<br><br>';
	}
	}
	//$html .= '</th></tr></table>';
	$html .= '</table>';
