...
// -----------------------------------------------------------------------------
//saco fases_figuras
$query = "select id_categoria, id_figura, id, orden from fases_figuras where id_competicion ='".$GLOBALS["id_competicion_activa"]."' and id_categoria = '".$id_categoria."' order by orden";
$fases_figuras = mysql_query($query);
while($fase_figura = mysql_fetch_array($fases_figuras)){
	$query = "select nombre from categorias where id = '".$fase_figura['id_categoria']."'"; 
    $nombre_categoria=mysql_result(mysql_query($query),0);
	$query = "select nombre from figuras where id = '".$fase_figura['id_figura']."'"; 
    $nombre_figura=mysql_result(mysql_query($query),0);
	$query = "select numero from figuras where id = '".$fase_figura['id_figura']."'"; 
    $numero_figura=mysql_result(mysql_query($query),0);
    //saco numeros de juez
   	...
   	while($juez = mysql_fetch_array($jueces)){
	   	...
	   	while($orden = mysql_fetch_array($ordenes)){		
			...
			$html .= '<tr><th colspan="2"><h4>'.$GLOBALS["nombre_competicion_activa"].'</h4></th></tr>';
			$html .= '<tr><th colspan="2">'.'<span>'.$nombre_categoria.'</span>'.'</th></tr>';
			$html .= '<tr><th colspan="2">'.'<span>'.$numero_figura.' - '.$nombre_figura.'</span>'.'</th></tr>';
			$html .= '<tr><th>'.'<span>ORDEN</span><br><span style="font-size:50px">'.$orden['orden'].'</span>'.'</th><th rowspan="2">'.'<span>NOTA</span>'.'</th></tr>';
			$html .= '<tr><th>'.'<span>JUEZ</span><br><span style="font-size:50px">'.$juez['numero_juez'].'</span></th></tr>';
			$html .= '<tr><th colspan="2" style="text-align:left; margin-left:5%;">'.'<span>&nbsp;&nbsp;FIRMA<br><br></span>'.'</th></tr>';
			$html .= '</table>';
