...
echo '<h1>Cierra esta página y actualiza la página de los paneles de jueces</h1>';
    ...
    while ($componentes = mysql_fetch_array($panel_jueces)){
		...
		$sql = "SELECT id from fases_figuras where id_competicion = ".$componentes['id_competicion'].' LIMIT 1,1000';
		...
		if(mysql_num_rows(mysql_query('select id_juez from panel_jueces where id_juez='.$componentes['id_juez'].' and id_competicion = '.$componentes['id_competicion'])) > 1 ) {
